...
from dienstleister.ml_lifecycle_utils.ml_lifecycle_subjects_name import _SAVE_MANY_RAW_DATA_SUBJECT_NAME, \
    ...
    _PROCESS_RAW_DATA_SUBJECT_NAME, _PREDICT_SUBJECT_NAME, _GET_LABELED_DATASET_SUBJECT_NAME, _GET_EDC_LABELED_DATASET_SUBJECT_NAME
...
async def request_save_many_raw_data_points(fiot_service: FastIoTService, data: list[dict],
    ...
    :py:data:`DB_SAVE_MANY_RAW_DATAPOINTS_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DB_SAVE_MANY_RAW_DATAPOINTS_SUBJECT>` \
    subject to save raw datapoints to the database utilizing the :func:`request_replysubject_thing_wrapper <hersteller.ml_lifecycle_utils.ml_lifecycle_broker_facade.request_replysubject_thing_wrapper>`
...
async def request_upsert_many_processed_data_points(fiot_service: FastIoTService, data: list[dict],
    ...
    """
    This function sends a request to the broker under the \
    :data:`DB_UPSERT_MANY_PROCESSED_DATAPOINTS_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DB_UPSERT_MANY_PROCESSED_DATAPOINTS_SUBJECT>` \
    subject to update processed datapoints in the database utilizing the :func:`request_replysubject_thing_wrapper <hersteller.ml_lifecycle_utils.ml_lifecycle_broker_facade.request_replysubject_thing_wrapper>`
...
async def request_get_all_raw_data_points(fiot_service: FastIoTService,
    ...
    """
    This function sends a request to the broker under the \
    :data:`DB_GET_ALL_RAW_DATA_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DB_GET_ALL_RAW_DATA_SUBJECT>` \
    for all raw datapoints in the database utilizing the :func:`request_replysubject_thing_wrapper <hersteller.ml_lifecycle_utils.ml_lifecycle_broker_facade.request_replysubject_thing_wrapper>`
...
async def request_get_all_processed_data_points(fiot_service: FastIoTService,
    ...
    """
    This function sends a request to the broker under the \
    :data:`DB_GET_ALL_PROCESSED_DATA_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DB_GET_ALL_PROCESSED_DATA_SUBJECT>` \
    for all processed datapoints in the database utilizing the \
    :func:`request_replysubject_thing_wrapper <hersteller.ml_lifecycle_utils.ml_lifecycle_broker_facade.request_replysubject_thing_wrapper>`
...
async def request_get_processed_data_points_count(fiot_service: FastIoTService,
    ...
    """
    This function sends a request to the broker under the \
    :data:`DB_GET_PROCESSED_DATA_POINTS_COUNT_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DB_GET_PROCESSED_DATA_POINTS_COUNT_SUBJECT>` \
...
async def request_get_processed_data_points_page(fiot_service: FastIoTService, page: int = 0, page_size: int = 10,
    ...
    """
    This function sends a request to the broker under the \
    :data:`DB_GET_PROCESSED_DATA_PAGE_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DB_GET_PROCESSED_DATA_PAGE_SUBJECT>` \
...
async def request_get_processed_data_points_from_raw_data(fiot_service: FastIoTService, data: dict,
    ...
    """
    This function sends a request to the broker under the \
    :data:`DATA_PROCESSING_PROCESS_RAW_DATA_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DATA_PROCESSING_PROCESS_RAW_DATA_SUBJECT>` \
...
async def request_get_prediction(fiot_service: FastIoTService, data: list[dict],
                                 timeout: float = 10) -> Future[list[dict]]:
    """
    This function sends a request to the broker under the \
    :data:`ML_SERVING_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.ML_SERVING_SUBJECT>` \
...
async def request_get_labeled_dataset(fiot_service: FastIoTService, data: dict = {},
    ...
    """
    This function sends a request to the broker under the \
    :data:`DB_GET_LABELED_DATASET_SUBJECT <hersteller.ml_lifecycle_utils.ml_lifecycle_subjects_name.DB_GET_LABELED_DATASET_SUBJECT>` \
...
async def request_get_edc_labeled_dataset(fiot_service: FastIoTService, data: dict = {},
    ...
    return await request_replysubject_thing_wrapper(
        fiot_service=fiot_service,
        data=data,
        subject=_GET_EDC_LABELED_DATASET_SUBJECT_NAME,  # same as the string "get-all-raw-datapoints"
        timeout=timeout,  # default is 10
    )